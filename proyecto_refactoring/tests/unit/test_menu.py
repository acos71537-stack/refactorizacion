...
def test_search_movie_empty_input_raises(
    favorites: FavoritesRepository, history: HistoryRepository
) -> None:
    output = io.StringIO()
    app = _build_app(favorites, history, output, ["1", "", "11"])
    app.run()
    assert "Entrada invalida" in output.getvalue()


def test_search_actor_empty_input_raises(
    favorites: FavoritesRepository, history: HistoryRepository
) -> None:
    output = io.StringIO()
    app = _build_app(favorites, history, output, ["2", "", "", "11"])
    app.run()
    assert "Entrada invalida" in output.getvalue()


def test_search_series_empty_input_raises(
    favorites: FavoritesRepository, history: HistoryRepository
) -> None:
    output = io.StringIO()
    app = _build_app(favorites, history, output, ["3", "", "", "11"])
    app.run()
    assert "Entrada invalida" in output.getvalue()


def test_by_genre_empty_input_raises(
    favorites: FavoritesRepository, history: HistoryRepository
) -> None:
    output = io.StringIO()
    app = _build_app(favorites, history, output, ["5", "", "11"])
    app.run()
    assert "Entrada invalida" in output.getvalue()
